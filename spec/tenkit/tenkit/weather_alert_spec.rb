...
RSpec.describe Tenkit::WeatherAlert do
  ...
  describe "weather_alert" do
    ...
    it "includes expected message" do
      ...
      expect(subject.messages.first.language).to eq "en"
      expect(subject.messages.first.text).to start_with "...HEAT ADVISORY"
      expect(subject.messages.first.text).to end_with "outdoor activities."
    end

    it "includes expected area feature" do
      expect(subject.area).to be_a Tenkit::Area
      expect(subject.area.type).to eq "FeatureCollection"
      expect(subject.area.features.first).to be_a Tenkit::Feature
      expect(subject.area.features.first.type).to eq "Feature"
      ...
      expect(subject.area.features.first.geometry.coordinates.size).to be 1
      expect(subject.area.features.first.geometry.coordinates.first.size).to be 177
